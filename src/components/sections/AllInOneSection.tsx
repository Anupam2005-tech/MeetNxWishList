      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="bg-background/10 backdrop-blur-sm p-6 sm:p-8 rounded-xl shadow-xl hover:shadow-2xl transition-shadow duration-300 border border-border/20 flex flex-col items-center text-center"
            >
              <div className="p-4 bg-primary/10 rounded-full mb-6">
                {benefit.icon}
              </div>
              <h3 className="font-headline text-xl sm:text-2xl font-semibold text-section-light-foreground mb-3">
                {benefit.title}
              </h3>
